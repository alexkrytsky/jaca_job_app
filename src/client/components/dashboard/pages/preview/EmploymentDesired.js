...
class EmploymentDesired extends Component {
  ...
  render() {
    ...
    return (
      <Grid container>
        <Grid item xs={12}>
          <p><AccountCircle />Applied For: <strong>{employmentDesired.employmentDesired}</strong>
          </p>
        </Grid>
        <Grid><MonetizationOn style={greenColor} /><strong>Salary
          Expectations:</strong> ${employmentDesired.salaryExpectations}</Grid>
        <Grid item xs={11}>
          <h4>{employmentDesired.applied ? <CheckCircle style={greenColor} /> :
            <Cancel style={redColor} />} Applied to MSC within the last 12 month</h4>
        </Grid>
        <Grid item xs={12}>
          <h4>{employmentDesired.workedAtMsc ? <CheckCircle style={greenColor} /> :
            <Cancel style={redColor} />} Worked at MSC before</h4>
        </Grid>
      </Grid>
    );
  }
